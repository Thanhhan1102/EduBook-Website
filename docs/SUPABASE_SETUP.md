...
5. Mở `/api/config` trên bản Vercel; khi cấu hình đúng sẽ thấy JSON có `url` và `publishableKey`. Mở `/register.html`, tạo tài khoản sinh viên mới, xác nhận email nếu được yêu cầu, rồi đăng nhập. Đặt thử một sách ở `/catalog.html` và kiểm tra đơn trong **Table Editor → orders, order_items**.
...
set role = 'admin'
where id = (select id from auth.users where email = 'admin-cua-ban@iuh.edu.vn');
```

Đăng xuất và đăng nhập lại. Tài khoản đó sẽ vào `/admin.html` để quản lý sách và yêu cầu. Người tự đăng ký luôn được gán vai trò `student`; giao diện không thể tự nâng quyền.
...
- Sinh viên chỉ đọc được hồ sơ và đơn của chính mình. Admin có thể xem các đơn, cập nhật trạng thái và quản lý sách. Khách chưa đăng nhập chỉ xem sách đang hoạt động.
