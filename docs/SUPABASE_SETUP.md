...
5. Mở `/api/config` trên bản Vercel; khi cấu hình đúng sẽ thấy JSON có `url` và `publishableKey`. Mở `/register.html`, tạo tài khoản bằng email hợp lệ (Gmail, Outlook, email trường...), xác nhận email nếu được yêu cầu, rồi đăng nhập. Website vẫn dùng email/mật khẩu, không cần bật Google OAuth. Đặt thử một sách ở `/catalog.html` và kiểm tra đơn trong **Table Editor → orders, order_items**.
